...
function potjans_neurons(scale = 1.0)
    ...
    for (k, v) in ccu
        if occursin("E", String(k))
            neurons[k] =
                SNN.AdEx(; N = 1, param = SNN.AdExParameter(; El = -49mV), name = string(k))

        else

            neurons[k] =
                SNN.IF(; N = 1, param = SNN.IFParameter(; El = -49mV), name = string(k))
        end
    end
    return neurons
end
...
display(
    plot(mean.(frE), xlabel = "Time [ms]", ylabel = "Firing rate [Hz]", legend = :topleft),
)
##
...
display(plot(pprob, pj, layout = (1, 2), size = (1000, 500), margin = 5SNNPlots.mm))
##
