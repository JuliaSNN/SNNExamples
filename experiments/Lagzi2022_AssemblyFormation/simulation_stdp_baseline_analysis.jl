...
W = mean(WW, dims=1)[1,:,:,:]
p1, p2 = plot(), plot()
p1=plot!(p1, NSSTs, W[1,:,1], label="Recurrent - $(synapses[1])", lw=3, title="Exc to Exc")
p1=plot!(NSSTs, W[1,:,2], label="Recurrent - $(synapses[2])", lw=3, title="Exc to Exc")
p1=plot!(NSSTs, W[2,:,1], label="Lateral - $(synapses[1])", lw=3, title="Exc to Exc")
p1=plot!(NSSTs, W[2,:,2], label="Lateral - $(synapses[2])", lw=3, title="Exc to Exc")

p2=plot!(p2, NSSTs, W[3,:,1], label="Recurrent - $(synapses[1])", lw=3, title="SST to Exc")
p2=plot!(NSSTs, W[3,:,2], label="Recurrent - $(synapses[2])", lw=3, title="SST to Exc")
p2=plot!(NSSTs, W[4,:,1], label="Lateral - $(synapses[1])", lw=3, title="SST to Exc")
p2=plot!(NSSTs, W[4,:,2], label="Lateral - $(synapses[2])", lw=3, title="SST to Exc")
...
for s in eachindex(synapses)
    for n in eachindex(NSSTs)
        ...
        model = data.model
        config = data.config
        raster(model.pop, 400s:405s, every=10, size=(800, 500), margin=5Plots.mm, link=:x, legend=:none, yrotation=0)

        plot_path = plotsdir("AssemblyLagzi2022", "baseline_model", "syn-$(syn)_nsst-$(nsst)") |> mkpath
