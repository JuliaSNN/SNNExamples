...
path = datadir("zeus","Lagzi2022_AssemblyFormation/mixed_inh", experiments[exp_name].name) |> mkpath

Threads.@threads for t in eachindex(NSSTs)
    local_config = (; config..., NSST = NSSTs[t])
    info = (NSST = NSSTs[t], τ= local_config.stim_τ, rate=local_config.stim_rate)
    model = nothing
    if isfile(get_path(path=path, name="Model_sst", info=info))
        @info "Loading model $(exp_name) with NSST = $(NSSTs[t])"
        model = load_model(path, "Model_sst", info).model
    else
        @info "Training model $(exp_name) with NSST = $(NSSTs[t])"
