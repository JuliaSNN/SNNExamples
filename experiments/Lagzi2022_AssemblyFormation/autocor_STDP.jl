...
Threads.@threads for n in eachindex(τs)
    ...
    E1 = Identity(N=200)
    E2 = Identity(N=200)
    signal1= SNN.PoissonStimulus(E1, :g, neurons = :ALL, N_pre=100, param = deepcopy(stim), name="ExtSignal_E1")
    signal2= SNN.PoissonStimulus(E2, :g, neurons = :ALL, N_pre=100, param = deepcopy(stim), name="ExtSignal_E1")
    W11 = SpikingSynapse(E1,E1, nothing,LTPParam= stdp_exc, p=0.2, μ=10)
    W12 = SpikingSynapse(E1,E2, nothing,LTPParam= stdp_exc, p=0.2, μ=10)
    W21 = SpikingSynapse(E2,E1, nothing,LTPParam= stdp_exc, p=.2, μ=10)
    W22 = SpikingSynapse(E2,E2, nothing,LTPParam= stdp_exc, p=0.2, μ=10)
...
plot(τs, ave_fr, lw=4, xlabel="τ (ms)", ylabel="Firing rate", title="Firing rate", grid=false, xscale=:log, guidefontsize=17, tickfontsize=10, size=(400,400))
...
plot!(τs, mean(W), ribbon=std(W), lw=4, label="", xscale=:log, xlabel="τ (ms)", ylabel="ΔW", title="Weight change", grid=false)

pal = palette(:roma, 10)
plot()
for n in 1:length(τs)
    fr1 = frs[n,1]
    fr2 = frs[n,2]
    crosscor(fr1,fr1, -200:200) |> x->plot!(-200:200, x, label="$(τs[n])", legendtitle="τ (ms)", color=pal[n], lw=3)
    crosscor(fr1,fr2, -200:200) |> x->plot!(-200:200, x, label="", legendtitle="τ (ms)", color=pal[n], lw=3, ls=:dash)
end
p2 = plot!(p2, grid=false, xlabel="Time (ms)", ylabel="Cross-correlation", title="Cross-correlation of firing rates", legend=:bottomright)
p = plot(p1, p2, size=(800, 400), margin=5Plots.mm, guidefontsize=17, tickfontsize=10, legendfontsize=10)
